...
class OrganProcess:
    ...
    def process_vec_organ(self, frame_state):
        ...
        if self.main_hero_info is None:
            return [0.0] * (self.unit_buff_num * 2 * self.one_unit_feature_num)
    ...
    def get_hp(self, organ, vector_feature):
        vector_feature.append(organ.get("hp", 0))

    def get_max_hp(self, organ, vector_feature):
        vector_feature.append(organ.get("max_hp", 0))

    def get_atk(self, organ, vector_feature):
        vector_feature.append(organ.get("phy_atk", organ.get("atk", 0)))

    def get_attack_range(self, organ, vector_feature):
        vector_feature.append(organ.get("attack_range", 0))
    ...
    def _distance_xz(self, pos1, pos2):
        if not pos1 or not pos2:
            return 0.0
        return math.sqrt((pos1.get("x", 0) - pos2.get("x", 0)) ** 2 + (pos1.get("z", 0) - pos2.get("z", 0)) ** 2)
    ...
    def get_dist_to_main_hero(self, organ, vector_feature):
        hero = self.main_hero_info or {}
        vector_feature.append(self._distance_xz(organ.get("location") or {}, hero.get("location") or {}))

    def get_dist_to_enemy_hero(self, organ, vector_feature):
        if self.enemy_hero_info is None:
            vector_feature.append(12300.0)
            return
        vector_feature.append(
            self._distance_xz(organ.get("location") or {}, self.enemy_hero_info.get("location") or {})
        )
